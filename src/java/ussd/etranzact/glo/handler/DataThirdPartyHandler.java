 ...
 */
public class DataThirdPartyHandler implements ShortCodeInterface {

    private final Logger LOG = Logger.getLogger(DataThirdPartyHandler.class);
    private String actionName = "Data-Third_Party";
    ...
    public String process(Data data) {
        ...
        if (session == null) {
            ...
            String[] msgLength = session.getQueue().get(0).split("\\*");// *389*805*1*<AMOUNT>*msisdn#

            int dataLen = msgLength.length;
            String data_Code = msgLength[dataLen - 4];
...
//                                if (verifyAccount) {
                                if (service.verifyUserPIN(data.getMsisdn(), subscriber.getAccountNo(), subscriber.getBankCode(), userPin, appCode)) {
                                    ...
                                    String resp1 = doMenubased3rdPartyDATAProcessing(session, subscriber.getBankCode(), subscriber.getAccountNo(), mobile3rdParty, amount, initialDial, productID, 0.0, userPin);
                                    sb.append(resp1);
                                    instance.destroySession(session.getSessionId());
                                    ...
                                    if (session.getSessionData().get("lock").equals("2") && session.getSessionData().get("lockCounter") == null) {
                                        ...
                                        if (service.verifyUserPIN(data.getMsisdn(), subscriber.getAccountNo(), subscriber.getBankCode(), message, appCode)) {
                                            ...
                                            String resp = doMenubased3rdPartyDATAProcessing(session, subscriber.getBankCode(), subscriber.getAccountNo(), mobile3rdParty, amount, initialDial, productID, 0.0, userPin);
                                            sb.append(resp);
                                            instance.destroySession(session.getSessionId());
                                        } else {
                    ...
                    if (session.getSessionData().get("pinn1") == null) {
                        ...
                        if (pinn1.equals(pinn2)) {
                            ...
                            if (verifyAccountA) {
                                ...
                                String initialDial = "1";//session.getQueue().get(1);
                                String resp1 = doMenubased3rdPartyDATAProcessing(session, bankCode, accountNo, mobile3rdParty, amount, initialDial, productID, 0.0, pinn1);
                                sb.append(resp1);
                                instance.destroySession(session.getSessionId());
                            } else {
                                sb.append("This mobile number is NOT tied to your bank account! ~ Please contact your bank for assistance.~ Thank you.").append("~");
...
//                            if (verifyAccount) {
                            if (service.verifyUserPIN(data.getMsisdn(), ums.getAccountNo(), userPin, appCode)) {
                                 ...
                                 */
                                String initialDial = "1";//session.getQueue().get(1);
                                String resp1 = doMenubased3rdPartyDATAProcessing(session, ums.getBankCode(), ums.getAccountNo(), mobile3rdParty, amount, initialDial, productID, 0.0, "");
                                sb.append(resp1);
                                instance.destroySession(session.getSessionId());
                            } else {
                                ...
                                if (session.getSessionData().get("lock").equals("2") && session.getSessionData().get("lockCounter") == null) {
                                    ...
                                    if (service.verifyUserPIN(data.getMsisdn(), ums.getAccountNo(), message, appCode)) {
                                        ...
                                        String resp = doMenubased3rdPartyDATAProcessing(session, ums.getBankCode(), ums.getAccountNo(), mobile3rdParty, amount, initialDial, productID, 0.0, "");
                                        sb.append(resp);
                                        instance.destroySession(session.getSessionId());
                                    } else {
                                        sb.append("Wrong PIN ~You have only 1 attempt left.~Enter PIN").append("~");
...
//                            }
     ...
     * //****************************************************************************************************************
     */
    private String doMenubased3rdPartyDATAProcessing(USSDSession session, String bankCode, String accountNo, String phoneNumber, String amount, String initialDial, String productID, double rate, String ussdPin) {
        phoneNumber = phoneNumber.replaceAll(" ", "");
        if (phoneNumber.startsWith("0")) {
            phoneNumber = "234" + phoneNumber.substring(1);
        }

        System.out.println("doMenubased3rdPartyDATAProcessing Details of doProcessing....");
        System.out.println("doMenubased3rdPartyDATAProcessing MSISDN : " + session.getMsisdn());
        System.out.println("doMenubased3rdPartyDATAProcessing 3rd Party MSISDN : " + phoneNumber);
        System.out.println("doMenubased3rdPartyDATAProcessing Account Number : " + accountNo);
        System.out.println("doMenubased3rdPartyDATAProcessing Bank Code : " + bankCode);
        System.out.println("doMenubased3rdPartyDATAProcessing Amount : " + amount);
        System.out.println("doMenubased3rdPartyDATAProcessing ProductID : " + productID);
        System.out.println("doMenubased3rdPartyDATAProcessing Initial Dial : " + initialDial);
        LOG.info("doMenubased3rdPartyDATAProcessing Calling doProcessing!!!!");
        ...
        String actionName = "DATA-THIRD_PARTY";
            ...
            try {
                switch (xResponse.getResponse()) {
                    case 0:
                        ...
                        String merchantCode = "JT_" + bankCode + "_805";//GloService.getInstance().findMerchantCode(bankCode);
